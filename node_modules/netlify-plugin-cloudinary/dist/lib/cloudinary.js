...
exports.getTransformationsFromInputs = exports.updateHtmlImagesToCloudinary = exports.getCloudinaryUrl = exports.createPublicId = exports.configureCloudinary = exports.getCloudinary = void 0;
...
async function getCloudinaryUrl(options) {
    const { deliveryType, folder, path: filePath, localDir, remoteHost, transformations = [], uploadPreset, } = options;
    ...
    if (!cloudName) {
        throw new Error(`[Cloudinary] ${errors_1.ERROR_CLOUD_NAME_REQUIRED}`);
    }
    if (deliveryType === 'upload' && !canSignUpload && !uploadPreset) {
        if (!uploadPreset) {
            throw new Error(`[Cloudinary] ${errors_1.ERROR_UPLOAD_PRESET}`);
        }
        throw new Error(`[Cloudinary] ${errors_1.ERROR_API_CREDENTIALS_REQUIRED}`);
    }
    let fileLocation;
    let publicId;
    if (deliveryType === 'fetch') {
        ...
        for (let attempt = 0; attempt < maxAttempts; attempt++) {
            try {
                ...
                console.error(`[Cloudinary] Attempt ${attempt + 1} - ${errors_1.ERROR_ASSET_UPLOAD}`);
                ...
                if (attempt === maxAttempts - 1) {
                    ...
                    throw Error(errors_1.ERROR_ASSET_UPLOAD);
                }
                else {
...
async function updateHtmlImagesToCloudinary(html, options) {
    const { assets, deliveryType, uploadPreset, folder, localDir, remoteHost, loadingStrategy, transformations } = options;
    const errors = [];
 ...
 */
function getTransformationsFromInputs(inputs) {
...
exports.getTransformationsFromInputs = getTransformationsFromInputs;
